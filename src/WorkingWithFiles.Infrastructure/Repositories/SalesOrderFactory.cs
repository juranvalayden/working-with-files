    ...
    private static readonly ThreadLocal<Faker<SalesOrderDto>> _faker =
        new(() =>
        {
            ...
            f.RuleFor(s => s.Id, _ => (int)Interlocked.Increment(ref _globalId))
             ...
             .RuleFor(s => s.OrderDate, faker => faker.Date.Past(1))
             ...
             .RuleFor(s => s.TaxAmt, (faker, s) => Math.Round((s.SubTotal + s.Freight) * 0.15m, 2))
             .RuleFor(s => s.TotalDue, (faker, s) => s.SubTotal + s.Freight + s.TaxAmt)
             .RuleFor(s => s.RowGuid, faker => Guid.NewGuid())
             .RuleFor(s => s.ModifiedDate, faker => DateTime.UtcNow)
             .RuleFor(s => s.ShipDate, faker => faker.Date.Future(1))
